...
get_effects <- function(output, settings = 1) {
  effects <- lapply(output, function(out) {
    ...
    list(
      intercept = linear_struct$intercept,
      w = linear_struct$w,
      w_groups = linear_struct$w_all_groups
    )
  })
  effects
}
      ...
      for (w in 1:1) {
        for (d in 1:4) {
          for (g in 1:3) {
            ...
            beta.params.pct[[tt]][v, b, w, d, g, ] <- c(
              mean(params$a),
              mean(params$b)
            )
...
saveRDS(posterior.pct.quantiles, paste0(
  "posterior_pct_quantiles_",
  fam, ".rds"
))
